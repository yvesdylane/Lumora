...
async def generateCaptions(
    ...
    logger.info("generateCaptions raw LLM response: %s", raw[:500])

    try:
        result = _parseJson(raw)
...
async def suggestTransition(
    ...
    raw = await client.complete(systemPrompt, userPrompt)
    logger.info("suggestTransition raw LLM response: %s", raw[:500])
...
async def suggestCutPoints(
    ...
    raw = await client.complete(systemPrompt, userPrompt)
    logger.info("suggestCutPoints raw LLM response: %s", raw[:500])
...
async def generateMotionSpec(
    ...
    raw = await client.complete(systemPrompt, userPrompt)
    logger.info("generateMotionSpec raw LLM response: %s", raw[:500])
